...
public class SachDao {
    ...
    public static List<Sach> laydulieuSach(String ma) throws SQLException {
        ...
        while (rs.next()) {
            ...
            Sach s = new Sach(masach, tensach, hinhanh, matacgia, matl, sl, manxb, namxb1, file);
            list.add(s);
        }
        return list;
    }
    ...
    public static void capnhatSach(Sach object) {
        Sach s = object;
        String sql = "UPDATE SACH SET tensach = ?,hinhanh = ?,matacgia = ?,matl = ?,soluong = ?,manxb = ?,namxb = ?,files = ? WHERE masach = ?";
